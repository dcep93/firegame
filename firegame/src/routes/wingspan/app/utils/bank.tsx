import ActivationsBank from "./activations_bank";
import {
  BonusType,
  CardType,
  ColorEnum,
  ExpansionEnum,
  FoodEnum,
  HabitatEnum,
  NestEnum,
} from "./types";

const cards: CardType[] = [
  {
    activation: ActivationsBank.activationNone,
    name: "American Woodcock",
    scientific_name: "Scolopax minor",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 9,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 46,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
    ],
  },
  {
    activation: ActivationsBank.activationNone,
    name: "Blue-Winged Warbler",
    scientific_name: "Vermivora cyanoptera",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 20,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Food Web Expert",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.activationNone,
    name: "Hooded Warbler",
    scientific_name: "Setophaga citrina",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 18,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Food Web Expert",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.activationNone,
    name: "Prothonotary Warbler",
    scientific_name: "Protonotaria citrea",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.activationNone,
    name: "Trumpeter Swan",
    scientific_name: "Cygnus buccinator",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 9,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 203,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.activationNone,
    name: "Wild Turkey",
    scientific_name: "Meleagris gallopavo",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "",
    predator: false,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.ground,
    capacity: 5,
    wingspan: 135,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.allDraw,
    name: "Canvasback",
    scientific_name: "Aythya valisineria",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text: "All players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 82,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Bird Feeder",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Omnivore Expert",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.allDraw,
    name: "Northern Shoveler",
    scientific_name: "Spatula clypeata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 76,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.allDraw,
    name: "Purple Gallinule",
    scientific_name: "Porphyrio martinicus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.platform,
    capacity: 4,
    wingspan: 56,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Viticulturalist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.allDraw,
    name: "Spotted Sandpiper",
    scientific_name: "Actitis macularius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 38,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Enclosure Builder", "Food Web Expert", "Wetland Scientist"],
  },
  {
    activation: ActivationsBank.allDraw,
    name: "Wilson's Snipe",
    scientific_name: "Gallinago delicata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 41,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Enclosure Builder",
      "Food Web Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.allLay(NestEnum.bowl),
    name: "Lazuli Bunting",
    scientific_name: "Passerina amoena",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "All players lay 1 [egg] on any 1 [bowl] bird. You may lay 1 [egg] on 1 additional [bowl] bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Passerine Specialist",
      "Prairie Manager",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.allLay(NestEnum.cavity),
    name: "Pileated Woodpecker",
    scientific_name: "Dryocopus pileatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "All players lay 1 [egg] on any 1 [cavity] bird. You may lay 1 [egg] on 1 additional [cavity] bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 74,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Forester",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.allLay(NestEnum.ground),
    name: "Western Meadowlark",
    scientific_name: "Sturnella neglecta",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "All players lay 1 [egg] on any 1 [ground] bird. You may lay 1 [egg] on 1 additional [ground] bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 38,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.cacheOnForest,
    name: "Eurasian Nutcracker",
    scientific_name: "Nucifraga caryocatactes",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1-5 birds in your [forest]. Cache 1 [seed] from your supply on each.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 55,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Cartographer", "Bird Feeder", "Forester", "Wildlife Gardener"],
  },
  {
    activation: ActivationsBank.cachePerGrassland,
    name: "Eurasian Magpie",
    scientific_name: "Pica pica",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1 other player. For each action cube in their [grassland], cache 1 [wild] from the supply on any of your birds.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 56,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Omnivore Expert",
      "Platform Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.cachePerPredator,
    name: "Carrion Crow",
    scientific_name: "Corvus corone",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose any 1 player (including yourself). Cache 1 [rodent] from the supply on this bird for each [predator] that player has.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 92,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.cachePerPredator,
    name: "Griffon Vulture",
    scientific_name: "Gyps fulvus",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose any 1 player (including yourself). Cache 1 [rodent] from the supply on this bird for each [predator] that player has.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 252,
    habitats: [HabitatEnum.grassland],
    food: {},
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Large Bird Specialist",
      "Platform Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Carolina Chickadee",
    scientific_name: "Poecile carolinensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply and cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Coal Tit",
    scientific_name: "Periparus ater",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Gain 1 [seed] from the supply and cache it on this card. At any time, you may spend [seed] cached on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 6,
    wingspan: 16,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Eurasian Nuthatch",
    scientific_name: "Sitta europaea",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Gain 1 [seed] from the supply and cache it on this card. At any time, you may spend [seed] cached on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Juniper Titmouse",
    scientific_name: "Baeolophus ridgwayi",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply and cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Mountain Chickadee",
    scientific_name: "Poecile gambeli",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply and cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "Red-Breasted Nuthatch",
    scientific_name: "Sitta canadensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply and cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.cacheSeed,
    name: "White-Breasted Nuthatch",
    scientific_name: "Sitta carolinensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply and cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 28,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.chooseFoodEveryoneGains,
    name: "Bluethroat",
    scientific_name: "Luscinia svecica",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose a food type. All players gain 1 of that food from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.ground,
    capacity: 5,
    wingspan: 22,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.chooseFoodEveryoneGains,
    name: "Common Nightingale",
    scientific_name: "Luscinia megarhynchos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose a food type. All players gain 1 of that food from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.countDouble,
    name: "Cetti's Warbler",
    scientific_name: "Cettia cetti",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "This bird counts double toward the end-of-round goal, if it qualifies for the goal.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 17,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Feeder",
      "Passerine Specialist",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.countDouble,
    name: "Eurasian Green Woodpecker",
    scientific_name: "Picus viridis",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "This bird counts double toward the end-of-round goal, if it qualifies for the goal.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 48,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Bird Bander",
      "Bird Feeder",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.countDouble,
    name: "Greylag Goose",
    scientific_name: "Anser anser",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "This bird counts double toward the end-of-round goal, if it qualifies for the goal.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 164,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.cover2(HabitatEnum.forest),
    name: "Common Blackbird",
    scientific_name: "Turdus merula",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Place this bird sideways, so that it covers 2 [forest] spaces. Pay the lower egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 5,
    wingspan: 36,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Diet Specialist",
      "Forester",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.cover2(HabitatEnum.forest),
    name: "Long-Tailed Tit",
    scientific_name: "Aegithalos caudatus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Place this bird sideways, so that it covers 2 [forest] spaces. Pay the lower egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 18,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.cover2(HabitatEnum.grassland),
    name: "European Roller",
    scientific_name: "Coracias garrulus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Place this bird sideways, so that it covers 2 [grassland] spaces. Pay the lower egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 70,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.cover2(HabitatEnum.wetland),
    name: "Grey Heron",
    scientific_name: "Ardea cinerea",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Place this bird sideways, so that it covers 2 [wetland] spaces. Pay the lower egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 185,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.discardCardPlayForest,
    name: "Goldcrest",
    scientific_name: "Regulus regulus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard 1 [card] from your hand. If you do, play another bird in your [forest]. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 6,
    wingspan: 15,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Food Web Expert",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.discardEggDraw2,
    name: "Franklin's Gull",
    scientific_name: "Leucophaeus pipixcan",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard 1 [egg] to draw 2 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 91,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Bander",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Omnivore Expert",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.discardEggDraw2,
    name: "Killdeer",
    scientific_name: "Charadrius vociferus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard 1 [egg] to draw 2 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 46,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Enclosure Builder",
    ],
  },
  {
    activation: ActivationsBank.discardEggGainWild(1),
    name: "American Crow",
    scientific_name: "Corvus brachyrhynchos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Discard 1 [egg] from any of your other birds to gain 1 [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 99,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Bander",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.discardEggGainWild(1),
    name: "Black-Crowned Night-Heron",
    scientific_name: "Nycticorax nycticorax",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Discard 1 [egg] from any of your other birds to gain 1 [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 9,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 112,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.fish]: 1,
      [FoodEnum.rodent]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.discardEggGainWild(1),
    name: "Fish Crow",
    scientific_name: "Corvus ossifragus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Discard 1 [egg] from any of your other birds to gain 1 [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 91,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.discardEggGainWild(2),
    name: "Chihuahuan Raven",
    scientific_name: "Corvus cryptoleucus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Discard 1 [egg] from any of your other birds to gain 2 [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 112,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.rodent]: 1, [FoodEnum.wild]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Diet Specialist",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.discardEggGainWild(2),
    name: "Common Raven",
    scientific_name: "Corvus corax",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Discard 1 [egg] from any of your other birds to gain 2 [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 135,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 1, [FoodEnum.wild]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Diet Specialist",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.discardEggPlayForest,
    name: "Short-Toed Treecreeper",
    scientific_name: "Certhia brachydactyla",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard 1 [egg] from any bird. If you do, play another bird in your [forest]. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.discardFoodToTuck(FoodEnum.seed),
    name: "House Sparrow",
    scientific_name: "Passer domesticus",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Discard up to 5 [seed] from your supply. For each, tuck 1 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 6,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 22,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Nest Box Builder",
      "Passerine Specialist",
      "Prairie Manager",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.discardFoodToTuck(FoodEnum.wild),
    name: "Common Starling",
    scientific_name: "Sturnus vulgaris",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Discard up to 5 [wild] from your supply. For each, tuck 1 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 38,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.discardFoodToTuck(FoodEnum.wild),
    name: "Eurasian Collared-Dove",
    scientific_name: "Streptopelia decaocto",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Discard up to 5 [wild] from your supply. For each, tuck 1 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 52,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Cartographer",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Platform Builder",
      "Prairie Manager",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.discardFoodTuck(FoodEnum.invertebrate),
    name: "Common Swift",
    scientific_name: "Apus apus",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Discard up to 5 [invertebrate] from your supply. For each, tuck 1 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 46,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Counter",
      "Citizen Scientist",
      "Food Web Expert",
      "Nest Box Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.discardSeedLay2,
    name: "Thekla's Lark",
    scientific_name: "Galerida theklae",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard 1 [seed] from your supply. If you do, lay 2 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 34,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.discardTuck2(FoodEnum.fish),
    name: "American White Pelican",
    scientific_name: "Pelecanus erythrorhynchos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard a [fish] to tuck 2 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 274,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Bird Counter",
      "Citizen Scientist",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.discardTuck2(FoodEnum.fish),
    name: "Double-Crested Cormorant",
    scientific_name: "Phalacrocorax auritus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard [fish] to tuck 2 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 132,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Counter",
      "Citizen Scientist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.discardTuck2(FoodEnum.seed),
    name: "Black-Bellied Whistling Duck",
    scientific_name: "Dendrocygna autumnalis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard 1 [seed] to tuck 2 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 76,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.discardTuck2(FoodEnum.seed),
    name: "Canada Goose",
    scientific_name: "Branta canadensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard 1 [seed] to tuck 2 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 132,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Enclosure Builder",
      "Large Bird Specialist",
    ],
  },
  {
    activation: ActivationsBank.discardTuck2(FoodEnum.seed),
    name: "Sandhill Crane",
    scientific_name: "Antigone canadensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Discard 1 [seed] to tuck 2 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 196,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
    ],
  },
  {
    activation: ActivationsBank.discardWildPlayWetland,
    name: "Common Moorhen",
    scientific_name: "Gallinula chloropus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard 1 [wild] from your supply. If you do, play another bird in your [wetland]. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 6,
    wingspan: 53,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fish]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.draftNPlusOne,
    name: "American Oystercatcher",
    scientific_name: "Haematopus palliatus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text:
      "Draw [card] equal to the number of players +1. Starting with you and proceeding clockwise, each player selects 1 of those cards and places it in their hand. You keep the extra card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 81,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Enclosure Builder",
      "Food Web Expert",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.draw,
    name: "Mallard",
    scientific_name: "Anas platyrhynchos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Draw 1 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 89,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.draw2,
    name: "Black-Necked Stilt",
    scientific_name: "Himantopus mexicanus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 74,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Enclosure Builder",
      "Food Web Expert",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.draw2,
    name: "Carolina Wren",
    scientific_name: "Thryothorus ludovicianus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.draw2OthersDraw,
    name: "Savi's Warbler",
    scientific_name: "Locustella luscinioides",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Draw 2 [card]. All other players draw 1 [card] from the deck.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 20,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Food Web Expert",
      "Passerine Specialist",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.draw2Tuck1,
    name: "Audouin's Gull",
    scientific_name: "Ichthyaetus audouinii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Draw 2 [card] from the deck. Tuck 1 behind this bird and keep the other.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 1,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 132,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Counter",
      "Citizen Scientist",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawAllFaceup,
    name: "Brant",
    scientific_name: "Branta bernicla",
    color: ColorEnum.white,
    expansion: ExpansionEnum.swiftstart,
    text: "Draw the 3 face-up [card] in the bird tray.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 114,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Atlantic Puffin",
    scientific_name: "Fratercula arctica",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 8,
    nest: NestEnum.wild,
    capacity: 1,
    wingspan: 53,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Diet Specialist",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Bell's Vireo",
    scientific_name: "Vireo bellii",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 18,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Bander",
      "Enclosure Builder",
      "Food Web Expert",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "California Condor",
    scientific_name: "Gymnogyps californianus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 1,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 277,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: {},
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Enclosure Builder",
      "Large Bird Specialist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Cassin's Finch",
    scientific_name: "Haemorhous cassinii",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Cerulean Warbler",
    scientific_name: "Setophaga cerulea",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Chestnut-Collared Longspur",
    scientific_name: "Calcarius ornatus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 5,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 25,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Greater Prairie Chicken",
    scientific_name: "Tympanuchus cupido",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 5,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 71,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "King Rail",
    scientific_name: "Rallus elegans",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.platform,
    capacity: 6,
    wingspan: 51,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.fish]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Fishery Manager",
      "Omnivore Expert",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Painted Bunting",
    scientific_name: "Passerina ciris",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Diet Specialist",
      "Passerine Specialist",
      "Prairie Manager",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Red-Cockaded Woodpecker",
    scientific_name: "Picoides borealis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 36,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Forester",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Roseate Spoonbill",
    scientific_name: "Platalea ajaja",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 6,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 127,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fish]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Spotted Owl",
    scientific_name: "Strix occidentalis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 1,
    wingspan: 102,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Forester",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Sprague's Pipit",
    scientific_name: "Anthus spragueii",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 3,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Whooping Crane",
    scientific_name: "Grus americana",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 6,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 221,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(2),
    name: "Wood Stork",
    scientific_name: "Mycteria americana",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Draw 2 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 6,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 155,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.rodent]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonus(3),
    name: "Red-Crowned Crane",
    scientific_name: "Grus japonensis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.chinesepromo,
    text: "Draw 3 new bonus cards and keep 1.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 7,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 235,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fish]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonusDrawOrLay,
    name: "Little Bustard",
    scientific_name: "Tetrax tetrax",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Draw 1 new bonus card. Then gain 1 [card] or lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 110,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.drawBonusDrawOrLay,
    name: "Snowy Owl",
    scientific_name: "Bubo scandiacus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Draw 1 new bonus card. Then gain 1 [card] or lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 138,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.drawBonusGainDie,
    name: "Corsican Nuthatch",
    scientific_name: "Sitta whiteheadi",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text: "Draw 1 new bonus card. Then gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 21,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.drawBonusGainDieLayOrDraw,
    name: "European Turtle Dove",
    scientific_name: "Streptopelia turtur",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Draw 1 new bonus card. Then gain 1 [die] from the birdfeeder, lay 1 [egg] on any bird, or draw 1 [card].",
    predator: false,
    flocking: false,
    bonus: true,
    points: 4,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 51,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: ["Cartographer", "Bird Bander", "Bird Feeder", "Platform Builder"],
  },
  {
    activation: ActivationsBank.drawBonusdraw3Keep1,
    name: "Black-Tailed Godwit",
    scientific_name: "Limosa limosa",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text: "Draw 1 new bonus card. Then draw 3 [card] and keep 1 of them.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 6,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 76,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawBonusdraw3Keep1,
    name: "Red Knot",
    scientific_name: "Calidris canutus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text: "Draw 1 new bonus card. Then draw 3 [card] and keep 1 of them.",
    predator: false,
    flocking: false,
    bonus: true,
    points: 7,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 50,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Diet Specialist",
      "Enclosure Builder",
      "Food Web Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(1),
    name: "Black Tern",
    scientific_name: "Chlidonias niger",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 1 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 61,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Photographer",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(1),
    name: "Clark's Grebe",
    scientific_name: "Aechmophorus clarkii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 1 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 61,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(1),
    name: "Forster's Tern",
    scientific_name: "Sterna forsteri",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 1 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 79,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Historian",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(2),
    name: "Common Yellowthroat",
    scientific_name: "Geothlypis trichas",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 2 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 18,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Food Web Expert",
      "Passerine Specialist",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(2),
    name: "Pied-Billed Grebe",
    scientific_name: "Podilymbus podiceps",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 2 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.platform,
    capacity: 4,
    wingspan: 41,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Fishery Manager",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(2),
    name: "Red-Breasted Merganser",
    scientific_name: "Mergus serrator",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text:
      "Draw 2 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 78,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(2),
    name: "Ruddy Duck",
    scientific_name: "Oxyura jamaicensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 2 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.platform,
    capacity: 5,
    wingspan: 48,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.drawDiscard(2),
    name: "Wood Duck",
    scientific_name: "Aix sponsa",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Draw 2 [card]. If you do, discard 1 [card] from your hand at the end of your turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 76,
    habitats: [HabitatEnum.forest, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.drawPerEmpty,
    name: "Great Crested Grebe",
    scientific_name: "Podiceps cristatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Draw 1 [card] for each empty card slot in this row. At the end of your turn, keep 1 and discard the rest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 65,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.drawPerEmpty,
    name: "Wilson's Storm Petrel",
    scientific_name: "Oceanites oceanicus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Draw 1 [card] for each empty card slot in this row. At the end of your turn, keep 1 and discard the rest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.wild,
    capacity: 1,
    wingspan: 38,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Diet Specialist",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.fish),
    name: "Osprey",
    scientific_name: "Pandion haliaetus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players gain 1 [fish] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 160,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.fruit),
    name: "Baltimore Oriole",
    scientific_name: "Icterus galbula",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players gain 1 [fruit] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 9,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Diet Specialist",
      "Enclosure Builder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.fruit),
    name: "Black-Chinned Hummingbird",
    scientific_name: "Archilochus alexandri",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text: "All players gain a [fruit] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 8,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Omnivore Expert",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.invertebrate),
    name: "Eastern Phoebe",
    scientific_name: "Sayornis phoebe",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 28,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Enclosure Builder",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.invertebrate),
    name: "Scissor-Tailed Flycatcher",
    scientific_name: "Tyrannus forficatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 38,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Diet Specialist",
      "Prairie Manager",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGains(FoodEnum.seed),
    name: "Red Crossbill",
    scientific_name: "Loxia curvirostra",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "All players gain 1 [seed] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 28,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGainsFromFeeder,
    name: "Anna's Hummingbird",
    scientific_name: "Calypte anna",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Each player gains 1 [die] from the birdfeeder, starting with the player of your choice.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 13,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Bander",
      "Omnivore Expert",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.everyoneGainsFromFeeder,
    name: "Ruby-Throated Hummingbird",
    scientific_name: "Archilochus colubris",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Each player gains 1 [die] from the birdfeeder, starting with the player of your choice.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 10,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Omnivore Expert",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.evolve,
    name: "Common Buzzard",
    scientific_name: "Buteo buteo",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Instead of paying any costs, you may play this bird on top of another bird on your player mat. Discard any eggs and food from that bird. It becomes a tucked card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 123,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.evolve,
    name: "Eurasian Hobby",
    scientific_name: "Falco subbuteo",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Instead of paying any costs, you may play this bird on top of another bird on your player mat. Discard any eggs and food from that bird. It becomes a tucked card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 75,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: true,
    food_star: true,
    bonuses: [
      "Cartographer",
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.evolve,
    name: "Montagu's Harrier",
    scientific_name: "Circus pygargus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Instead of paying any costs, you may play this bird on top of another bird on your player mat. Discard any eggs and food from that bird. It becomes a tucked card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 113,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Historian",
      "Citizen Scientist",
      "Enclosure Builder",
      "Falconer",
      "Large Bird Specialist",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.evolve,
    name: "Red Kite",
    scientific_name: "Milvus milvus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "Instead of paying any costs, you may play this bird on top of another bird on your player mat. Discard any eggs and food from that bird. It becomes a tucked card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 157,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.fewestForestGainDie,
    name: "Hermit Thrush",
    scientific_name: "Catharus guttatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Player(s) with fewest [forest] birds gain 1 [die] from birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Enclosure Builder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.fewestWetlandDraw,
    name: "American Bittern",
    scientific_name: "Botaurus lentiginosus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Player(s) with the fewest [wetland] birds: draw 1 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 107,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.fish]: 1,
      [FoodEnum.rodent]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.fewestWetlandDraw,
    name: "Common Loon",
    scientific_name: "Gavia immer",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Player(s) with the fewest [wetland] birds: draw 1 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 117,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.gainAll(FoodEnum.fish),
    name: "Bald Eagle",
    scientific_name: "Haliaeetus leucocephalus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Gain all [fish] that are in the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 9,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 203,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.gainAll(FoodEnum.invertebrate),
    name: "Black Woodpecker",
    scientific_name: "Dryocopus martius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Gain all [invertebrate] that are in the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 70,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Diet Specialist",
      "Forester",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.gainAll(FoodEnum.invertebrate),
    name: "Northern Flicker",
    scientific_name: "Colaptes auratus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Gain all [invertebrate] that are in the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 51,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.gainDie,
    name: "White-Backed Woodpecker",
    scientific_name: "Dendrocopos leucotos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 39,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Forester",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.gainFaceup(HabitatEnum.grassland),
    name: "Common Little Bittern",
    scientific_name: "Ixobrychus minutus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Gain 1 face-up [card] that can live in [grassland].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 49,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Fishery Manager",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.gainFaceup(HabitatEnum.wetland),
    name: "Squacco Heron",
    scientific_name: "Ardeola ralloides",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Gain 1 face-up [card] that can live in [wetland].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 86,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Acorn Woodpecker",
    scientific_name: "Melanerpes formicivorus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 46,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: ["Bird Feeder", "Diet Specialist", "Forester", "Nest Box Builder"],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Blue Jay",
    scientific_name: "Cyanocitta cristata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 41,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Omnivore Expert",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Clark's Nutcracker",
    scientific_name: "Nucifraga columbiana",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 61,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Red-Bellied Woodpecker",
    scientific_name: "Melanerpes carolinus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 41,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Red-Headed Woodpecker",
    scientific_name: "Melanerpes erythrocephalus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 43,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Nest Box Builder",
      "Omnivore Expert",
    ],
  },
  {
    activation: ActivationsBank.gainFeederSeedOptionalCache,
    name: "Steller's Jay",
    scientific_name: "Cyanocitta stelleri",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [seed] from the birdfeeder (if available). You may cache it on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 48,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Omnivore Expert",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.fish, 3),
    name: "Brown Pelican",
    scientific_name: "Pelecanus occidentalis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Gain 3 [fish] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 201,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.fruit),
    name: "Northern Cardinal",
    scientific_name: "Cardinalis cardinalis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [fruit] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 5,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.invertebrate),
    name: "Blue-Gray Gnatcatcher",
    scientific_name: "Polioptila caerulea",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 15,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Food Web Expert",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.invertebrate),
    name: "Painted Whitestart",
    scientific_name: "Myioborus pictus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text: "Gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 22,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Enclosure Builder",
      "Food Web Expert",
      "Forester",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.invertebrate),
    name: "Yellow-Bellied Sapsucker",
    scientific_name: "Sphyrapicus varius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 41,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Forester",
      "Nest Box Builder",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.seed),
    name: "Spotted Towhee",
    scientific_name: "Pipilo maculatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 28,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.gainFood(FoodEnum.seed, 3),
    name: "American Goldfinch",
    scientific_name: "Spinus tristis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Gain 3 [seed] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Passerine Specialist",
      "Prairie Manager",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFromFeeder([
      FoodEnum.invertebrate,
      FoodEnum.fruit,
    ]),
    name: "Indigo Bunting",
    scientific_name: "Passerina cyanea",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [invertebrate] or [fruit] from the birdfeeder, if there is one.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 20,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFromFeeder([
      FoodEnum.invertebrate,
      FoodEnum.fruit,
    ]),
    name: "Western Tanager",
    scientific_name: "Piranga ludoviciana",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Gain 1 [invertebrate] or [fruit] from the birdfeeder, if there is one.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Diet Specialist",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFromFeeder([FoodEnum.invertebrate]),
    name: "Great Crested Flycatcher",
    scientific_name: "Myiarchus crinitus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [invertebrate] from the birdfeeder, if there is one.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 33,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Anatomist", "Forester", "Nest Box Builder", "Viticulturalist"],
  },
  {
    activation: ActivationsBank.gainFromFeeder([FoodEnum.seed, FoodEnum.fruit]),
    name: "Rose-Breasted Grosbeak",
    scientific_name: "Pheucticus ludovicianus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Gain 1 [seed] or [fruit] from the birdfeeder, if there is one.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 33,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFromFeederAtEnd([
      FoodEnum.invertebrate,
      FoodEnum.fruit,
    ]),
    name: "Eurasian Golden Oriole",
    scientific_name: "Oriolus oriolus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.european,
    text:
      'When another player takes the "gain food" action, gain 1 [invertebrate] or [fruit] from the birdfeeder at the end of their turn.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 46,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.fruit]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Bird Bander",
      "Diet Specialist",
      "Enclosure Builder",
      "Nest Box Builder",
      "Omnivore Expert",
      "Platform Builder",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainFromFeederAtEnd([FoodEnum.seed]),
    name: "Eurasian Tree Sparrow",
    scientific_name: "Passer montanus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.european,
    text:
      'When another player takes the "gain food" action, gain 1 [seed] from the birdfeeder at the end of their turn.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 21,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.gainInvertegrateOnForest,
    name: "Eastern Kingbird",
    scientific_name: "Tyrannus tyrannus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player plays a [forest] bird, gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 38,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainRodentOnGainRodent,
    name: "Loggerhead Shrike",
    scientific_name: "Lanius ludovicianus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "gain food" action, if they gain any number of [rodent], also gain 1 [rodent] from the supply and cache it on this card.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 30,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Bander",
      "Passerine Specialist",
      "Rodentologist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.gainWildFromFeeder,
    name: "American Redstart",
    scientific_name: "Setophaga ruticilla",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text: "Gain 1 [wild] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(100),
    name: "Golden Eagle",
    scientific_name: "Aquila chrysaetos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <100cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 201,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Citizen Scientist",
      "Diet Specialist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(100),
    name: "Great Horned Owl",
    scientific_name: "Bubo virginianus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <100cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 112,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Citizen Scientist",
      "Diet Specialist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(100),
    name: "Peregrine Falcon",
    scientific_name: "Falco peregrinus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <100cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 104,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(50),
    name: "Greater Roadrunner",
    scientific_name: "Geococcyx californianus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <50cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 56,
    habitats: [HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.rodent]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Citizen Scientist",
      "Diet Specialist",
      "Falconer",
      "Omnivore Expert",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Barred Owl",
    scientific_name: "Strix varia",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 107,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Citizen Scientist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Cooper's Hawk",
    scientific_name: "Accipiter cooperii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 79,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Citizen Scientist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Northern Harrier",
    scientific_name: "Circus cyaneus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 109,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Red-Shouldered Hawk",
    scientific_name: "Buteo lineatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it under this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 102,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Citizen Scientist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Red-Tailed Hawk",
    scientific_name: "Buteo jamaicensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 124,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntDraw(75),
    name: "Swainson's Hawk",
    scientific_name: "Buteo swainsoni",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Look at a [card] from the deck. If <75cm, tuck it behind this bird. If not, discard it.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 130,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Citizen Scientist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Anhinga",
    scientific_name: "Anhinga anhinga",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 114,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Black Skimmer",
    scientific_name: "Rynchops niger",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 112,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Enclosure Builder",
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Common Merganser",
    scientific_name: "Mergus merganser",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 86,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Northern Gannet",
    scientific_name: "Morus bassanus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Roll all dice not in birdfeeder. If any are a [fish], gain that many [fish] from the supply and cache them on this bird.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 173,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Enclosure Builder",
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Snowy Egret",
    scientific_name: "Egretta thula",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 104,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Photographer",
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "White-Faced Ibis",
    scientific_name: "Plegadis chihi",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 91,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Diet Specialist",
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.fish),
    name: "Willet",
    scientific_name: "Tringa semipalmata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [fish], gain 1 [fish] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 66,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Enclosure Builder",
      "Falconer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "American Kestrel",
    scientific_name: "Falco sparverius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 56,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Falconer",
      "Nest Box Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Barn Owl",
    scientific_name: "Tyto alba",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 107,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Falconer",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Broad-Winged Hawk",
    scientific_name: "Buteo platypterus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text:
      "Roll all dice not in birdfeeder. If any are a [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 85,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Burrowing Owl",
    scientific_name: "Athene cunicularia",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 53,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Enclosure Builder",
      "Falconer",
      "Nest Box Builder",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Eastern Screech Owl",
    scientific_name: "Megascops asio",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 51,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Falconer",
      "Forester",
      "Nest Box Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Ferruginous Hawk",
    scientific_name: "Buteo regalis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 142,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.huntRoll(FoodEnum.rodent),
    name: "Mississippi Kite",
    scientific_name: "Ictinia mississippiensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Roll all dice not in birdfeeder. If any are [rodent], gain 1 [rodent] and cache it on this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 79,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Bander",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.layAny,
    name: "Baird's Sparrow",
    scientific_name: "Ammodramus bairdii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 23,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layAny,
    name: "Cassin's Sparrow",
    scientific_name: "Peucaea cassinii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 20,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layAny,
    name: "Chipping Sparrow",
    scientific_name: "Spizella passerina",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.layAny,
    name: "Grasshopper Sparrow",
    scientific_name: "Ammodramus savannarum",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on any bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 20,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.bowl]),
    name: "Bronzed Cowbird",
    scientific_name: "Molothrus aeneus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [bowl] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.none,
    capacity: 0,
    wingspan: 36,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Photographer", "Bird Feeder", "Prairie Manager"],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.bowl]),
    name: "Brown-Headed Cowbird",
    scientific_name: "Molothrus ater",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [bowl] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.none,
    capacity: 0,
    wingspan: 30,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.bowl]),
    name: "Yellow-Billed Cuckoo",
    scientific_name: "Coccyzus americanus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [bowl] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 46,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Diet Specialist",
      "Forester",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.cavity]),
    name: "Barrow's Goldeneye",
    scientific_name: "Bucephala islandica",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [cavity] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 71,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fish]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Historian",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Nest Box Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.ground, FoodEnum.bowl]),
    name: "Common Cuckoo",
    scientific_name: "Cuculus canorus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.european,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [bowl] or [ground] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.none,
    capacity: 0,
    wingspan: 57,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: ["Bird Bander", "Food Web Expert"],
  },
  {
    activation: ActivationsBank.layEgg([FoodEnum.ground]),
    name: "American Avocet",
    scientific_name: "Recurvirostra americana",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      'When another player takes the "lay eggs" action, this bird lays 1 [egg] on another bird with a [ground] nest.',
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 79,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.layForEachCavity,
    name: "Common Goldeneye",
    scientific_name: "Bucephala clangula",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Lay 1 [egg] on this bird for each other bird with a [cavity] nest that you have.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 73,
    habitats: [HabitatEnum.forest, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Fishery Manager",
      "Large Bird Specialist",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.layOnAll(NestEnum.bowl),
    name: "Say's Phoebe",
    scientific_name: "Sayornis saya",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on each of your birds with a [bowl] nest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 33,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Diet Specialist",
      "Food Web Expert",
      "Prairie Manager",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.layOnAll(NestEnum.cavity),
    name: "Ash-Throated Flycatcher",
    scientific_name: "Myiarchus cinerascens",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on each of your birds with a [cavity] nest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 30,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Diet Specialist",
      "Nest Box Builder",
      "Passerine Specialist",
      "Prairie Manager",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.layOnAll(NestEnum.ground),
    name: "Bobolink",
    scientific_name: "Dolichonyx oryzivorus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on each of your birds with a [ground] nest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 30,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layOnAll(NestEnum.platform),
    name: "Inca Dove",
    scientific_name: "Columbina inca",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on each of your birds with a [platform] nest.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.platform,
    capacity: 4,
    wingspan: 28,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Passerine Specialist",
      "Platform Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layOnColumn,
    name: "Red-Legged Partridge",
    scientific_name: "Alectoris rufa",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text: "Lay 1 [egg] on each bird in this column, including this one.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.ground,
    capacity: 6,
    wingspan: 48,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layOnEmptyHabitat,
    name: "Black Redstart",
    scientific_name: "Phoenicurus ochruros",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose a habitat with no [egg]. Lay 1 [egg] on each bird in that habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 24,
    habitats: [HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Prairie Manager",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.layOnEmptyHabitat,
    name: "Lesser Whitethroat",
    scientific_name: "Sylvia curruca",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose a habitat with no [egg]. Lay 1 [egg] on each bird in that habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 18,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.layPerGrassland,
    name: "Dunnock",
    scientific_name: "Prunella modularis",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1 other player. For each action cube on their [grassland], lay 1 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.layThis,
    name: "California Quail",
    scientific_name: "Callipepla californica",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 6,
    wingspan: 36,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
    ],
  },
  {
    activation: ActivationsBank.layThis,
    name: "Mourning Dove",
    scientific_name: "Zenaida macroura",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.platform,
    capacity: 5,
    wingspan: 46,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.layThis,
    name: "Northern Bobwhite",
    scientific_name: "Colinus virginianus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Lay 1 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 6,
    wingspan: 33,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Bird Feeder",
      "Diet Specialist",
      "Enclosure Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.layThis,
    name: "Scaled Quail",
    scientific_name: "Callipepla squamata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text: "Lay 1 [egg] on this bird.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.ground,
    capacity: 6,
    wingspan: 36,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Bewick's Wren",
    scientific_name: "Thryomanes bewickii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 18,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Blue Grosbeak",
    scientific_name: "Passerina caerulea",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 28,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Bird Feeder",
      "Diet Specialist",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Chimney Swift",
    scientific_name: "Chaetura pelagica",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 36,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Enclosure Builder",
      "Food Web Expert",
      "Nest Box Builder",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Common Nighthawk",
    scientific_name: "Chordeiles minor",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 56,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Enclosure Builder",
      "Food Web Expert",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Lincoln's Sparrow",
    scientific_name: "Melospiza lincolnii",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 20,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Song Sparrow",
    scientific_name: "Melospiza melodia",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 0,
    nest: NestEnum.bowl,
    capacity: 5,
    wingspan: 20,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "White-Crowned Sparrow",
    scientific_name: "Zonotrichia leucophrys",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 5,
    wingspan: 25,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.migrate,
    name: "Yellow-Breasted Chat",
    scientific_name: "Icteria virens",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "If this bird is to the right of all other birds in its habitat, move it to another habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Bander",
      "Diet Specialist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playAndGain(HabitatEnum.wetland, FoodEnum.fish),
    name: "Belted Kingfisher",
    scientific_name: "Megaceryle alcyon",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player plays a [wetland] bird, gain 1 [fish] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 4,
    wingspan: 53,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Omnivore Expert",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playAnotherIfUsed4,
    name: "Moltoni's Warbler",
    scientific_name: "Sylvia subalpina",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "If you used all 4 types of actions this round, play another bird. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 17,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Passerine Specialist",
      "Prairie Manager",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playAnotherIfUsed4,
    name: "White Wagtail",
    scientific_name: "Motacilla alba",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "If you used all 4 types of actions this round, play another bird. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.bowl,
    capacity: 5,
    wingspan: 28,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Food Web Expert",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playAnotherIfUsed4,
    name: "Yellowhammer",
    scientific_name: "Emberiza citrinella",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "If you used all 4 types of actions this round, play another bird. Pay its normal food and egg cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 26,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([
      HabitatEnum.forest,
      HabitatEnum.grassland,
    ]),
    name: "House Wren",
    scientific_name: "Troglodytes aedon",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text:
      "Play a second bird in your [grassland] or [forest]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 15,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Food Web Expert",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.forest]),
    name: "Downy Woodpecker",
    scientific_name: "Picoides pubescens",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [forest]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.forest]),
    name: "Red-Eyed Vireo",
    scientific_name: "Vireo olivaceus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [forest]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 2,
    wingspan: 25,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Enclosure Builder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.forest]),
    name: "Ruby-Crowned Kinglet",
    scientific_name: "Regulus calendula",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [forest]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.forest]),
    name: "Tufted Titmouse",
    scientific_name: "Baeolophus bicolor",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [forest]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.grassland]),
    name: "Eastern Bluebird",
    scientific_name: "Sialia sialis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [grassland]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 33,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Nest Box Builder",
      "Prairie Manager",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.grassland]),
    name: "Mountain Bluebird",
    scientific_name: "Sialia currucoides",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [grassland]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 5,
    wingspan: 36,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Nest Box Builder",
      "Prairie Manager",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.grassland]),
    name: "Savannah Sparrow",
    scientific_name: "Passerculus sandwichensis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [grassland]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 18,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Feeder",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.wetland]),
    name: "Great Blue Heron",
    scientific_name: "Ardea herodias",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [wetland]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 183,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.playSecond([HabitatEnum.wetland]),
    name: "Great Egret",
    scientific_name: "Ardea alba",
    color: ColorEnum.white,
    expansion: ExpansionEnum.core,
    text: "Play a second bird in your [wetland]. Pay its normal cost.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 130,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.predatorGainFood,
    name: "Black Vulture",
    scientific_name: "Coragyps atratus",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player's predator succeeds, gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 1,
    wingspan: 150,
    habitats: [HabitatEnum.forest],
    food: {},
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Forester",
      "Large Bird Specialist",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.predatorGainFood,
    name: "Black-Billed Magpie",
    scientific_name: "Pica hudsonia",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player's predator succeeds, gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.wild,
    capacity: 3,
    wingspan: 64,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.wild]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Enclosure Builder",
      "Nest Box Builder",
      "Omnivore Expert",
      "Platform Builder",
      "Prairie Manager",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.predatorGainFood,
    name: "Turkey Vulture",
    scientific_name: "Cathartes aura",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player's predator succeeds, gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 1,
    nest: NestEnum.cavity,
    capacity: 1,
    wingspan: 170,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: {},
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Large Bird Specialist",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.reduceCost,
    name: "Bonelli's Eagle",
    scientific_name: "Aquila fasciata",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "For each [rodent] in this bird's cost, you may pay 1 [card] from your hand instead. If you do, tuck the paid [card] behind this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 160,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.rodent]: 3 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Historian",
      "Bird Bander",
      "Citizen Scientist",
      "Diet Specialist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.reduceCost,
    name: "Eastern Imperial Eagle",
    scientific_name: "Aquila heliaca",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "For each [rodent] in this bird's cost, you may pay 1 [card] from your hand instead. If you do, tuck the paid [card] behind this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 7,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 200,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.rodent]: 3 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Cartographer",
      "Citizen Scientist",
      "Diet Specialist",
      "Falconer",
      "Large Bird Specialist",
      "Platform Builder",
      "Prairie Manager",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.reduceCost,
    name: "Eurasian Sparrowhawk",
    scientific_name: "Accipiter nisus",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "For each [rodent] in this bird's cost, you may pay 1 [card] from your hand instead. If you do, tuck the paid [card] behind this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 65,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Cartographer",
      "Citizen Scientist",
      "Falconer",
      "Forester",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.reduceCost,
    name: "Northern Goshawk",
    scientific_name: "Accipiter gentilis",
    color: ColorEnum.white,
    expansion: ExpansionEnum.european,
    text:
      "For each [rodent] in this bird's cost, you may pay 1 [card] from your hand instead. If you do, tuck the paid [card] behind this card.",
    predator: true,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 106,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.rodent]: 2 },
    food_slash: false,
    food_star: true,
    bonuses: [
      "Cartographer",
      "Citizen Scientist",
      "Falconer",
      "Forester",
      "Large Bird Specialist",
      "Platform Builder",
      "Rodentologist",
    ],
  },
  {
    activation: ActivationsBank.regainFood,
    name: "European Robin",
    scientific_name: "Erithacus rubecula",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "From the supply, gain 1 food of a type you already gained this turn.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 22,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.removeDieGainSeed,
    name: "Parrot Crossbill",
    scientific_name: "Loxia pytyopsittacus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Remove any 1 [die] from the birdfeeder, then gain 1 [seed] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 3 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.repeatBrown,
    name: "Gray Catbird",
    scientific_name: "Dumetella carolinensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Repeat a brown power on one other bird in this habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 28,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Diet Specialist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.repeatBrown,
    name: "Northern Mockingbird",
    scientific_name: "Mimus polyglottos",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Repeat a brown power on one other bird in this habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 2,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 36,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.repeatPredator,
    name: "Hooded Merganser",
    scientific_name: "Lophodytes cucullatus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Repeat 1 [predator] power in this habitat.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 61,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Fishery Manager", "Nest Box Builder", "Wetland Scientist"],
  },
  {
    activation: ActivationsBank.rerollRodentToLay,
    name: "Eleonora's Falcon",
    scientific_name: "Falco eleonorae",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Roll all dice not in the birdfeeder. If any are [rodent], place 1 [egg] on this card.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 6,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 95,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Rodentologist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.resetFeederGain([
      FoodEnum.seed,
      FoodEnum.fruit,
    ]),
    name: "Bullfinch",
    scientific_name: "Pyrrhula pyrrhula",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Reset the birdfeeder. If you do, gain 1 [seed] or [fruit] from the birdfeeder after resetting.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 24,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.resetFeederGainAll(FoodEnum.invertebrate),
    name: "European Honey Buzzard",
    scientific_name: "Pernis apivorus",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Reset the birdfeeder. If you do, gain all [invertebrate] in the birdfeeder after resetting.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 1,
    wingspan: 134,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Forester",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.resetFeederGainOr([FoodEnum.invertebrate]),
    name: "European Bee-Eater",
    scientific_name: "Merops apiaster",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Reset the birdfeeder. If you do, gain 1 [invertebrate] from the birdfeeder after resetting.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 38,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Food Web Expert",
      "Nest Box Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.resetFeederGainOr([FoodEnum.seed]),
    name: "Hawfinch",
    scientific_name: "Coccothraustes coccothraustes",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Reset the birdfeeder. If you do, gain 1 [seed] from the birdfeeder after resetting.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 30,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.resetFeederGainWild,
    name: "Great Tit",
    scientific_name: "Parus major",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Reset the birdfeeder. If you do, gain 1 [die] from the birdfeeder after resetting.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 6,
    wingspan: 24,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.stealFood(FoodEnum.fish),
    name: "Common Kingfisher",
    scientific_name: "Alcedo atthis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Steal 1 [fish] from another player's supply and cache it on this bird. They gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.wild,
    capacity: 5,
    wingspan: 26,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Diet Specialist",
      "Enclosure Builder",
      "Fishery Manager",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.stealFood(FoodEnum.invertebrate),
    name: "Red-Backed Shrike",
    scientific_name: "Lanius collurio",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Steal 1 [invertebrate] from another player's supply and cache it on this bird. They gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 33,
    habitats: [HabitatEnum.grassland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.rodent]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Diet Specialist",
      "Omnivore Expert",
      "Prairie Manager",
      "Rodentologist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.stealFood(FoodEnum.rodent),
    name: "Little Owl",
    scientific_name: "Athene noctua",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Steal 1 [rodent] from another player's supply and cache it on this bird. They gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 56,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.rodent]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: ["Bird Bander", "Nest Box Builder", "Rodentologist"],
  },
  {
    activation: ActivationsBank.stealFood(FoodEnum.seed),
    name: "Eurasian Jay",
    scientific_name: "Garrulus glandarius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Steal 1 [seed] from another player's supply and cache it on this bird. They gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 56,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Bird Feeder",
      "Diet Specialist",
      "Forester",
      "Omnivore Expert",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.stealFood(FoodEnum.wild, true),
    name: "Black-Headed Gull",
    scientific_name: "Chroicocephalus ridibundus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Steal 1 [wild] from another player's supply and add it to your own supply. They gain 1 [die] from the birdfeeder.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 102,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tradeFood,
    name: "Green Heron",
    scientific_name: "Butorides virescens",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text: "Trade 1 [wild] for any [wild] from the supply.",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.platform,
    capacity: 3,
    wingspan: 66,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Photographer",
      "Fishery Manager",
      "Large Bird Specialist",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.trashTrayDraw1,
    name: "Black-Throated Diver",
    scientific_name: "Gavia arctica",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard all remaining face-up [card] and refill the tray. If you do, draw 1 of the new face-up [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 120,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.fish]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Enclosure Builder",
      "Fishery Manager",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.trashTrayDraw1,
    name: "White Stork",
    scientific_name: "Ciconia ciconia",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard all remaining face-up [card] and refill the tray. If you do, draw 1 of the new face-up [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 8,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 160,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.fish]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Bird Bander",
      "Diet Specialist",
      "Fishery Manager",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.trashTrayDraw1,
    name: "White-Throated Dipper",
    scientific_name: "Cinclus cinclus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Discard all remaining face-up [card] and refill the tray. If you do, draw 1 of the new face-up [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 28,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fish]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Fishery Manager",
      "Nest Box Builder",
      "Passerine Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "American Coot",
    scientific_name: "Fulica americana",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 5,
    wingspan: 61,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Omnivore Expert",
      "Platform Builder",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "American Robin",
    scientific_name: "Turdus migratorius",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 43,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Barn Swallow",
    scientific_name: "Hirundo rustica",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 1,
    nest: NestEnum.wild,
    capacity: 3,
    wingspan: 38,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Enclosure Builder",
      "Food Web Expert",
      "Nest Box Builder",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "House Finch",
    scientific_name: "Haemorhous mexicanus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 6,
    wingspan: 25,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Purple Martin",
    scientific_name: "Progne subis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 46,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Food Web Expert",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Ring-Billed Gull",
    scientific_name: "Larus delawarensis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 122,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.wild]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Bird Counter",
      "Citizen Scientist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Tree Swallow",
    scientific_name: "Tachycineta bicolor",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 38,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Counter",
      "Citizen Scientist",
      "Nest Box Builder",
      "Viticulturalist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Violet-Green Swallow",
    scientific_name: "Tachycineta thalassina",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 36,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Food Web Expert",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDraw,
    name: "Yellow-Rumped Warbler",
    scientific_name: "Setophaga coronata",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, draw 1 [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 1,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.forest],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Forester",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndDrawAtEnd,
    name: "Snow Bunting",
    scientific_name: "Plectrophenax nivalis",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.european,
    text:
      "When another player tucks a [card] for any reason, tuck 1 [card] from your hand behind this bird, then draw 1 [card] at the end of their turn.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 35,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.tuckAndGainFood([FoodEnum.fruit]),
    name: "Cedar Waxwing",
    scientific_name: "Bombycilla cedrorum",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, gain 1 [fruit] from the supply.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.fruit]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndGainFood([
      FoodEnum.invertebrate,
      FoodEnum.seed,
    ]),
    name: "Pygmy Nuthatch",
    scientific_name: "Sitta pygmaea",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, gain 1 [invertebrate] or 1 [seed] from the supply.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 4,
    wingspan: 20,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Forester",
      "Nest Box Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndGainFood([FoodEnum.invertebrate]),
    name: "Vaux's Swift",
    scientific_name: "Chaetura vauxi",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, gain 1 [invertebrate] from the supply.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 3,
    wingspan: 31,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Backyard Birder",
      "Bird Counter",
      "Citizen Scientist",
      "Food Web Expert",
      "Forester",
      "Nest Box Builder",
    ],
  },
  {
    activation: ActivationsBank.tuckAndGainFood([FoodEnum.seed]),
    name: "Dark-Eyed Junco",
    scientific_name: "Junco hyemalis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, gain 1 [seed] from the supply.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Enclosure Builder",
      "Passerine Specialist",
    ],
  },
  {
    activation: ActivationsBank.tuckAndGainFood([FoodEnum.seed]),
    name: "Pine Siskin",
    scientific_name: "Spinus pinus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, gain 1 [seed] from the supply.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 23,
    habitats: [HabitatEnum.forest],
    food: { [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Forester",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Brewer's Blackbird",
    scientific_name: "Euphagus cyanocephalus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 41,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Historian",
      "Photographer",
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Omnivore Expert",
      "Prairie Manager",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Bushtit",
    scientific_name: "Psaltriparus minimus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.wild,
    capacity: 5,
    wingspan: 15,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Enclosure Builder",
      "Nest Box Builder",
      "Passerine Specialist",
      "Platform Builder",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Common Grackle",
    scientific_name: "Quiscalus quiscula",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 43,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Omnivore Expert",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Dickcissel",
    scientific_name: "Spiza americana",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 25,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Diet Specialist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Red-Winged Blackbird",
    scientific_name: "Agelaius phoeniceus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 33,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLay,
    name: "Yellow-Headed Blackbird",
    scientific_name: "Xanthocephalus xanthocephalus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.core,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, also lay 1 [egg] on this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 3,
    wingspan: 38,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Wetland Scientist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckAndLayAnywhere,
    name: "White-Throated Swift",
    scientific_name: "Aeronautes saxatalis",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.swiftstart,
    text:
      "Tuck a [card] from your hand behind this bird. If you do, lay 1 [egg] on any bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.cavity,
    capacity: 2,
    wingspan: 38,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Anatomist",
      "Photographer",
      "Backyard Birder",
      "Bird Counter",
      "Citizen Scientist",
      "Food Web Expert",
      "Nest Box Builder",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.tuckBehindHabitat,
    name: "Common Chaffinch",
    scientific_name: "Fringilla coelebs",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1-5 birds in this habitat. Tuck 1 [card] from your hand behind each.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 4,
    nest: NestEnum.bowl,
    capacity: 2,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Passerine Specialist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckBehindHabitat,
    name: "Common Chiffchaff",
    scientific_name: "Phylloscopus collybita",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1-5 birds in this habitat. Tuck 1 [card] from your hand behind each.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 20,
    habitats: [HabitatEnum.forest, HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.fruit]: 1,
    },
    food_slash: true,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Viticulturalist",
    ],
  },
  {
    activation: ActivationsBank.tuckMultipleWetland,
    name: "Mute Swan",
    scientific_name: "Cygnus olor",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1-3 birds in your [wetland]. Tuck 1 [card] from your hand behind each. If you tuck at least 1 card, draw 1 [card].",
    predator: false,
    flocking: false,
    bonus: false,
    points: 4,
    nest: NestEnum.ground,
    capacity: 3,
    wingspan: 220,
    habitats: [HabitatEnum.wetland],
    food: {
      [FoodEnum.invertebrate]: 1,
      [FoodEnum.seed]: 1,
      [FoodEnum.wild]: 1,
    },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Feeder",
      "Citizen Scientist",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckOnGrassland,
    name: "Horned Lark",
    scientific_name: "Eremophila alpestris",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.core,
    text:
      "When another player plays a [grassland] bird, tuck 1 [card] from your hand behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 5,
    nest: NestEnum.ground,
    capacity: 4,
    wingspan: 30,
    habitats: [HabitatEnum.grassland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Enclosure Builder",
      "Passerine Specialist",
      "Prairie Manager",
    ],
  },
  {
    activation: ActivationsBank.tuckOnTuck,
    name: "European Goldfinch",
    scientific_name: "Carduelis carduelis",
    color: ColorEnum.pink,
    expansion: ExpansionEnum.european,
    text:
      "When another player tucks a [card] for any reason, tuck 1 [card] from the deck behind this bird.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 6,
    nest: NestEnum.bowl,
    capacity: 4,
    wingspan: 23,
    habitats: [HabitatEnum.forest, HabitatEnum.grassland],
    food: { [FoodEnum.seed]: 2, [FoodEnum.fruit]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Cartographer",
      "Photographer",
      "Bird Bander",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Diet Specialist",
      "Passerine Specialist",
      "Viticulturalist",
      "Wildlife Gardener",
    ],
  },
  {
    activation: ActivationsBank.tuckPer(HabitatEnum.grassland),
    name: "Hooded Crow",
    scientific_name: "Corvus cornix",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1 other player. For each action cube on their [grassland], tuck 1 [card] from your hand behind this bird, then draw an equal number of [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.platform,
    capacity: 2,
    wingspan: 99,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Diet Specialist",
      "Large Bird Specialist",
      "Omnivore Expert",
      "Platform Builder",
    ],
  },
  {
    activation: ActivationsBank.tuckPer(HabitatEnum.wetland),
    name: "Greater Flamingo",
    scientific_name: "Phoenicopterus roseus",
    color: ColorEnum.brown,
    expansion: ExpansionEnum.european,
    text:
      "Choose 1 other player. For each action cube on their [wetland], tuck 1 [card] from your hand behind this bird, then draw an equal number of [card].",
    predator: false,
    flocking: true,
    bonus: false,
    points: 3,
    nest: NestEnum.ground,
    capacity: 1,
    wingspan: 152,
    habitats: [HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.seed]: 2 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Counter",
      "Bird Feeder",
      "Citizen Scientist",
      "Diet Specialist",
      "Enclosure Builder",
      "Large Bird Specialist",
      "Wetland Scientist",
    ],
  },
  {
    activation: ActivationsBank.tuckUpTo3AndDraw,
    name: "Ruff",
    scientific_name: "Calidris pugnax",
    color: ColorEnum.teal,
    expansion: ExpansionEnum.european,
    text:
      "Tuck up to 3 [card] from your hand behind this bird. Draw 1 [card] for each card you tucked.",
    predator: false,
    flocking: true,
    bonus: false,
    points: 2,
    nest: NestEnum.ground,
    capacity: 2,
    wingspan: 56,
    habitats: [HabitatEnum.grassland, HabitatEnum.wetland],
    food: { [FoodEnum.invertebrate]: 1, [FoodEnum.wild]: 1 },
    food_slash: false,
    food_star: false,
    bonuses: [
      "Backyard Birder",
      "Bird Bander",
      "Bird Counter",
      "Citizen Scientist",
      "Enclosure Builder",
      "Omnivore Expert",
    ],
  },
];

const bonuses: BonusType[] = [
  {
    name: "Anatomist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with body parts in their names",
    extra:
      "Body parts include back, beak, belly, bill, breast, cap, chin, collar, crest, crown, eye, face, head, leg, neck, rump, shoulder, tail, throat, toe, wing",
    vp_text: "2 to 3 birds: 3; 4+ birds: 7",
    vp_f: { 2: 3, 4: 7 },
    percent: 22,
  },
  {
    name: "Backyard Birder",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: false,
    automa_only: false,
    condition: "Birds worth less than 4 points",
    extra: "",
    vp_text: "5 to 6 birds: 3; 6+ birds: 6",
    vp_f: { 5: 3, 6: 6 },
    percent: 42,
  },
  {
    name: "Behaviorist",
    to_skip: true, // todo
    expansion: ExpansionEnum.european,
    automa: false,
    automa_only: false,
    condition:
      "For each column that contains birds with 3 different power colors:",
    extra: "Birds with no power count as white.",
    vp_text: "3 per column",
    vp_f: { 0: 3 },
    percent: null,
  },
  {
    name: "Bird Bander",
    to_skip: false,
    expansion: ExpansionEnum.european,
    automa: true,
    automa_only: false,
    condition: "Birds that can live in multiple habitats",
    extra: "",
    vp_text: "4 to 5 birds: 4; 6+ birds: 7",
    vp_f: { 4: 4, 6: 7 },
    percent: 31,
  },
  {
    name: "Bird Counter",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with a [flocking] power",
    extra: "",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 15,
  },
  {
    name: "Bird Feeder",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat [seed]",
    extra: "",
    vp_text: "5 to 7 birds: 3; 8+ birds: 7",
    vp_f: { 5: 3, 8: 7 },
    percent: 44,
  },
  {
    name: "Breeding Manager",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: false,
    automa_only: false,
    condition: "Birds that have at least 4 eggs laid on them",
    extra: "",
    vp_text: "1 per bird",
    vp_f: { 0: 1 },
    percent: 28,
  },
  {
    name: "Cartographer",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with geography terms in their name",
    extra:
      "Terms include American, Atlantic, Baltimore, California, Canada, Carolina, Chihuahua, Corsican, Eastern, Eurasian, European, Inca, Mississippi, Moor, Mountain, Northern, Prairie, Sandhill, Savannah, Western",
    vp_text: "2 to 3 birds: 3; 4+ birds: 7",
    vp_f: { 2: 3, 4: 7 },
    percent: 21,
  },
  {
    name: "Citizen Scientist",
    to_skip: false,
    expansion: ExpansionEnum.european,
    automa: true,
    automa_only: false,
    condition: "Birds with tucked cards",
    extra: "",
    vp_text: "4 to 6 birds: 3; 7+ birds: 6",
    vp_f: { 4: 3, 7: 6 },
    percent: 21,
  },
  {
    name: "Diet Specialist",
    to_skip: false,
    expansion: ExpansionEnum.european,
    automa: true,
    automa_only: false,
    condition: "Birds with a food cost of 3 food",
    extra: "",
    vp_text: "2 to 3 birds: 3; 4+ birds: 6",
    vp_f: { 2: 3, 4: 6 },
    percent: 29,
  },
  {
    name: "Ecologist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: false,
    automa_only: false,
    condition: "Birds in your habitat with the fewest birds.",
    extra: "Ties count.",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: null,
  },
  {
    name: "Enclosure Builder",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with [ground] nests",
    extra: "Birds must have a [ground] or [star] nest symbol.",
    vp_text: "4 to 5 birds: 4; 6+ birds: 7",
    vp_f: { 4: 4, 6: 7 },
    percent: 31,
  },
  {
    name: "Ethologist",
    to_skip: true, // todo
    expansion: ExpansionEnum.european,
    automa: false,
    automa_only: false,
    condition: "In any one habitat:",
    extra: "Birds with no power count as white.",
    vp_text: "2 per power color",
    vp_f: { 0: 2 },
    percent: null,
  },
  {
    name: "Falconer",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with a [predator] power",
    extra: "",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 13,
  },
  {
    name: "Fishery Manager",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat [fish]",
    extra: "",
    vp_text: "2 to 3 birds: 3; 4+ birds: 8",
    vp_f: { 2: 3, 4: 8 },
    percent: 18,
  },
  {
    name: "Food Web Expert",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat only [invertebrate]",
    extra: "",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 9,
  },
  {
    name: "Forester",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that can only live in [forest]",
    extra: "",
    vp_text: "3 to 4 birds: 4; 5 birds: 5",
    vp_f: { 3: 4, 5: 5 },
    percent: 24,
  },
  {
    name: "Historian",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds named after a person",
    extra: "Any bird with an 's in its name.",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 11,
  },
  {
    name: "Large Bird Specialist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with wingspans over 65 cm",
    extra: "",
    vp_text: "4 to 5 birds: 3; 6+ birds: 6",
    vp_f: { 4: 3, 6: 6 },
    percent: 35,
  },
  {
    name: "Nest Box Builder",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with [cavity] nests",
    extra: "Birds must have a [cavity] or [star] nest symbol.",
    vp_text: "4 to 5 birds: 4; 6+ birds: 7",
    vp_f: { 4: 4, 6: 7 },
    percent: 31,
  },
  {
    name: "Omnivore Specialist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat [wild]",
    extra:
      "Any bird that specifically has a [wild] symbol as part of its food cost.",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 16,
  },
  {
    name: "Oologist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: false,
    automa_only: false,
    condition: "Birds that have at least 1 egg laid on them",
    extra: "",
    vp_text: "7 to 8 birds: 3; 9+ birds: 6",
    vp_f: { 7: 3, 9: 6 },
    percent: null,
  },
  {
    name: "Passerine Specialist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with wingspans 30 cm or less",
    extra: "",
    vp_text: "4 to 5 birds: 3; 6+ birds: 6",
    vp_f: { 4: 3, 6: 6 },
    percent: 35,
  },
  {
    name: "Photographer",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with colors in their names",
    extra:
      "Colors include ash, black, blue, bronze, brown, cerulean, chestnut, coal, ferruginous, gold, gray, grey, green, honey, indigo, lazuli, purple, red, rose, roseate, ruby, ruddy, rufous, snowy, violet, white, yellow",
    vp_text: "4 to 5 brids: 3; 6+ birds: 7",
    vp_f: { 4: 3, 6: 7 },
    percent: 34,
  },
  {
    name: "Platform Builder",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with [platform] nests",
    extra: "Birds must have a [platform] or [star] nest symbol.",
    vp_text: "4 to 5 birds: 4; 6+ birds: 7",
    vp_f: { 4: 4, 6: 7 },
    percent: 31,
  },
  {
    name: "Prairie Manager",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that can only live in [grassland]",
    extra: "",
    vp_text: "2 to 3 birds: 3; 4+ birds: 8",
    vp_f: { 2: 3, 4: 8 },
    percent: 19,
  },
  {
    name: "Rodentologist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat [rodent]",
    extra: "",
    vp_text: "2 per bird",
    vp_f: { 0: 2 },
    percent: 15,
  },
  {
    name: "Visionary Leader",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: false,
    automa_only: false,
    condition: "Bird cards in hand at end of game",
    extra: "",
    vp_text: "5 to 7 birds: 4; 8+ birds: 7",
    vp_f: { 5: 4, 8: 7 },
    percent: null,
  },
  {
    name: "Viticulturalist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that eat [fruit]",
    extra: "",
    vp_text: "2 to 3 birds: 3; 4+ birds: 7",
    vp_f: { 2: 3, 4: 7 },
    percent: 22,
  },
  {
    name: "Wetland Scientist",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds that can only live in [wetland]",
    extra: "",
    vp_text: "3 to 4 birds: 3; 5 birds: 7",
    vp_f: { 3: 3, 5: 7 },
    percent: 26,
  },
  {
    name: "Wildlife Gardener",
    to_skip: false,
    expansion: ExpansionEnum.core,
    automa: true,
    automa_only: false,
    condition: "Birds with [bowl] nests",
    extra: "Birds must have a [bowl] or [star] nest symbol.",
    vp_text: "4 to 5 birds: 4; 6+ birds: 7",
    vp_f: { 4: 4, 6: 7 },
    percent: 31,
  },
  {
    name: "[automa] Autwitcher",
    to_skip: false,
    expansion: ExpansionEnum.european,
    automa: true,
    automa_only: true,
    condition: "Birds that are worth 3 or 4 points",
    extra: "The automa keeps up to 2 of them (higher value first).",
    vp_text: "-",
    vp_f: null,
    percent: 44,
  },
  {
    name: "[automa] RASPB Life Fellow",
    to_skip: false,
    expansion: ExpansionEnum.european,
    automa: true,
    automa_only: true,
    condition: "Birds that are worth 5, 6, or 7 points",
    extra: "The automa keeps the highest valued one.",
    vp_text: "-",
    vp_f: null,
    percent: 28,
  },
];

const bank = { cards, bonuses };

export default bank;
