...
class TokenBank extends React.Component<PropsType, { [t in Token]?: boolean }> {
              ...
              (token) =>
                token !== Token.gold && (
                  <div
                    ...
                    key={token}
                    className={`${this.isSelected(token) && styles.grey}`}
                  >
  ...
  select(token: Token) {
    ...
    if (!this.isSelected(token)) {
      if (Object.values(this.state).filter(Boolean).length === MAX_TOKENS) {
  ...
  take() {
    ...
    this.clearSelect();
    utils.finishTurn(`took ${toTake.map((t) => TokenToEmoji[t]).join(", ")}`);
  }
}
