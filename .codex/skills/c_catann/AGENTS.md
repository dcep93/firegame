...
- Late single-player knight replay after client sequence 306 now requires largest-army state/log updates in the sequence-77 GameStateUpdated diff, then an immediate `31 -> 32 -> 30` highlight chain; in this environment an extra `HighlightTiles` clear (`type 33`, sequence 81) still appears before the next robber click and causes desync.
- Late single-player knight replay after client action 306 may require additional empty highlight clears beyond sequence 80; recent runs matched through sequence 82 and then expected another `HighlightTiles` clear (`type 33`, sequence 83).
- Winner finalization now needs to emit, in order, win-log `GameStateUpdated` (type 91), `CanResignGame` false (type 6), and `GameEndState` (type 45) to align with single_player sequence 86-88.
- `RequestBeginnerModeLevelEnd` (client action 64) must be treated as handled in `applyGameAction`; expected follow-up currently includes lobby state update (`id: 133`, type 3) and a beginner-hint update (type 77, payload `{ type: 48, data: 11 }`).
- Current blocker after sequence 89: an extra malformed client message (`{ payload: 15 }`) appears with a browser bridge error (`reading 'receive'`), indicating response-shape/order issues in the post-game action-64 handling.
...
- Current branch behavior: implementing non-bank `CreateTrade` handling in `gameLogic/index.ts` removes the immediate `not implemented 123` crash in `2p.v1`, but regression still blocks earlier in `2p.v0` on initial-placement canvas clickability (`clickCanvas` poll timeout).
