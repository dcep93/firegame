...
When an `api/player` response gives the current player a normal action prompt,
select `Pass for this generation` by default without submitting it or
reselecting it during later UI updates.

## Qualifying Network Response

Only a successful captured `api/player` player-view response can arm the
default. A response qualifies when its serialized `waitingFor` model:

- is an `or` choice;
- has the title `Take your first action` or `Take your next action`; and
- contains a `Pass for this generation` option.

Captured `player/input` responses never arm the default. Nonqualifying
`api/player` responses clear any stale pending default.
...
pass detection, network requests, or behavior after `player/input` responses.
...
- `api/player` responses can arm the default while `player/input` responses
  cannot;
