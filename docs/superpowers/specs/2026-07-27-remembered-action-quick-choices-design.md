...
For an action-panel submission:
...
The same canonical-submit capture helper serves both visible Actions panels:

- a trusted click on the canonical submit reaches it through the document
  listener; and
- a trusted user click on the cloned submit reaches it explicitly through the
  mirror proxy, using that clone element's mapped canonical submit.

The synthetic canonical click generated by the mirror proxy remains ineligible
in the document listener, so the choice is staged once. Other synthetic clicks,
autopilot execution, and quick-choice playback remain ineligible and cannot
teach the extension.
...
- staging choices submitted through the cloned Actions panel;
- rejecting synthetic canonical clicks except through the explicit trusted
  mirror-proxy path;
